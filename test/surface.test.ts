...
describe("product surface", () => {
  it("publishes four focused Japanese pages", async () => {
    ...
    for (const path of ["/", "/guide", "/source", "/privacy"]) {
      const response = await app.request(`https://kyujin-joken.yhay81.com${path}`, undefined, env);
  ...
  it("keeps the product visual, bounded, and free of internal meta language", async () => {
    const response = await app.request("https://kyujin-joken.yhay81.com/", undefined, bindings());
    const html = await response.text();
    const surface = html + readFileSync(resolve(process.cwd(), "public/styles.css"), "utf8");
    expect(html).toContain("condition-board");
    expect(html).toContain("完全週休二日");
    expect(html).toContain("事業所件数の割合");
  ...
  it("accepts only same-origin allowlisted anonymous events", async () => {
    ...
    const accepted = await app.request(
      "https://kyujin-joken.yhay81.com/api/telemetry",
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
          origin: "https://kyujin-joken.yhay81.com",
          "x-kyujin-joken-session": "12345678-1234-4123-8123-123456789abc",
          "x-kyujin-joken-qa": "1",
        },
  ...
  it("separates automated QA and honors browser privacy controls", () => {
    ...
    expect(client).toContain('"x-kyujin-joken-qa"');
    expect(migration).toContain("is_qa");
    expect(migration).toContain("metric_changed");
  });

  it("publishes exactly four canonical sitemap URLs", async () => {
    const response = await app.request(
      "https://kyujin-joken.yhay81.com/sitemap.xml",
      undefined,
      bindings(),
    );
    ...
    expect(locations).toContain("https://kyujin-joken.yhay81.com/source");
  });

  it("returns a product 404 for unknown paths", async () => {
    const response = await app.request(
      "https://kyujin-joken.yhay81.com/missing",
      undefined,
      bindings(),
    );
    expect(response.status).toBe(404);
    expect(await response.text()).toContain("この票は見つかりません");
  });
});
